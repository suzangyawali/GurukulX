...
//course ko instance banyo ki banena
  if (!course) {
    return next(
      new AppError('Course could not be created, please try again', 400)
    );
  }
// couse ko instance banyo vane
  // Run only if user sends a file
  if (req.file) {
    try {
      const result = await cloudinary.v2.uploader.upload(req.file.path, {
        folder: 'gurukulx', // Save files in a folder named gurukulx
      });
...
export const deleteCourseById = asyncHandler(async (req, res, next) => {
  ...
  // Remove course (updated to use findByIdAndDelete instead of deprecated remove())
